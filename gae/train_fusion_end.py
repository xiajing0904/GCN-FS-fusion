...
def test(model, loader, test_score, device, roi_num):
    ...
    out_sum1 = torch.tensor(()).to(device)
    ...
    for data in loader:  # Iterate in batches over the training/test dataset.
        ...
        pre_loss1 = F.cross_entropy(out1, test_tem, w, size_average=False)
        pre_loss2 = F.cross_entropy(out2, test_tem, w,size_average=False)
        ...
        out_sum1 = torch.cat((out_sum1, out1), dim=0) 
        out_sum2 = torch.cat((out_sum2, out2), dim=0) 
        pre_loss_sum = pre_loss_sum + pre_loss
        pre_loss1_sum = pre_loss1_sum + pre_loss1
        pre_loss2_sum = pre_loss2_sum + pre_loss2
    ...
    out1 = np.squeeze(np.asarray(out_sum1.cpu().detach().numpy()))
    ...
    accu1 =  np.sum(out1 == truev)/truev.shape[0]
    accu2 =  np.sum(out2 == truev)/truev.shape[0]
    attention = np.squeeze(np.asarray(attention.cpu().detach().numpy())) 
    pre_loss_sum = pre_loss_sum/count
    pre_loss1_sum = pre_loss1_sum/count
    pre_loss2_sum = pre_loss2_sum/count
    #print(attention.shape)     
    return pre_loss_sum, pre_loss1_sum, pre_loss2_sum, accu,accu1,accu2, out, out1, out2, truev, attention
...
hc2 = 256 
hc3 = hidden_channels*roi_num*2#14848 #464#14848 #29696# 14848 #3712 #1856#14848
...
sconnpath2 = dir +'/dataset_SC/PD/X.npy'  
...
for X_train, X_test in stratifiedKFolds.split(all_data_fc, all_score): #kf.split(list(range(0,num))): #
    ...
    for epoch in range(1, epoch_num):
        ...
        pre_loss_sum, pre_loss1_sum, pre_loss2_sum, accu, accu11,accu12,train_out, train_out1, train_out2, train_true, train_att  = test(model, train_loader,train_score_input, device, roi_num)
        pre_loss_sum2, pre_loss1_sum2, pre_loss2_sum2, accu2, accu21,accu22,test_out, test_out1, test_out2, test_true, test_att  = test(model, test_loader,test_score_input, device, roi_num)
        if epoch % 1 == 0:
            print(epoch)
            print(f'Epoch: {epoch:03d}, Train_loss: {pre_loss_sum:.4f}, Train_accu: {accu:.4f}, Train_accu1: {accu11:.4f}, Train_accu2: {accu12:.4f}')
            print(f'Epoch: {epoch:03d}, Test_loss: {pre_loss_sum2:.4f}, Test_accu: {accu2:.4f}, Test_accu1: {accu21:.4f}, Test_accu2: {accu22:.4f}')
            #pre_loss_sum = torch.tensor(np.float32(pre_loss_sum))
            #pre_loss_sum2 = torch.tensor(np.float32(pre_loss_sum2))
            accu2 = torch.tensor(np.float32(accu2))
            accu21 = torch.tensor(np.float32(accu21))
            accu22 = torch.tensor(np.float32(accu22))
            if accuu < accu2:
                accuu, accuu1, accuu2 = accu2, accu21, accu22
                train_attention = train_att
                test_attention = test_att
                test_outf = test_out
                test_truef = test_true
                epochf = epoch
                
    write_attention(timefile, train_attention, fold)
    ...
    test_corr_s.append(accuu)
    pred_out_best = np.concatenate((pred_out_best, test_outf), axis=0)
    true_out_best = np.concatenate((true_out_best, test_truef), axis=0)
    test_att_all = np.concatenate((test_att_all,test_attention), axis = 0)
